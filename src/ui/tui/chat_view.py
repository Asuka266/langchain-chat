...
async def start_chat(app) -> None:
    ...
    while True:
        ...
        if user_input.startswith("/"):
            result = await _handle_command(app, session, user_input)
            if result == "exit":
                break
...
async def _create_new_session(app):
    """创建新会话（含预设选择）。返回 Session 或 None。"""
    widgets.console.print("\n[bold]=== 新建会话 ===[/bold]")
...
async def _select_preset(app):
    """选择预设。返回 preset_id 或 None。"""
...
async def _handle_command(app, session, command: str) -> str:
    """处理对话中的 / 命令。返回 "exit" / "new_session" / None。"""
